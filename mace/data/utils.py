...
class Configuration:
    ...
    is_interlayer_config: bool = False
...
def config_from_atoms_list(
    ...
    is_interlayer_atoms_list: bool = False,
    interlayer_atoms_key: str = "layer_ids",
) -> Configurations:
    ...
    for atoms in atoms_list:
        all_configs.append(
            config_from_atoms(
                ...
                interlayer_atoms_key=interlayer_atoms_key,
            )
        )
    return all_configs
...
def config_from_atoms(
    ...
    config_type_weights: Optional[Dict[str, float]] = None,
    head_name: str = "Default",
    is_interlayer_atoms: bool = False,
    interlayer_atoms_key: str = "layer_ids",
) -> Configuration:
    ...
    if is_interlayer_atoms and layer_ids is None:
        raise ValueError(
            f"Interlayer configurations require per-atom '{interlayer_atoms_key}' data"
        )
...
def load_from_xyz(
    ...
    interlayer_xyz_file: bool = False,
    interlayer_atoms_key: str = "layer_ids",
    ...
    if interlayer_xyz_file:
        for idx, atoms in enumerate(atoms_list):
            if interlayer_atoms_key not in atoms.arrays:
                raise ValueError(
                    f"Interlayer XYZ file '{file_path}' config {idx} is missing "
                    f"per-atom '{interlayer_atoms_key}' data"
                )
...
def save_configurations_as_HDF5(configurations: Configurations, _, h5_file) -> None:
    ...
    for j, config in enumerate(configurations):
        ...
        subgroup["is_interlayer_config"] = write_value(config.is_interlayer_config)
        if config.layer_ids is not None:
            subgroup["layer_ids"] = write_value(config.layer_ids)
