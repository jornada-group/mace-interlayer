...
class HDF5Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        config = Configuration(
            ...
            is_interlayer_config=bool(
                unpack_value(subgrp.get("is_interlayer_config", False)[()])
            )
            if "is_interlayer_config" in subgrp
            else False,
            layer_ids=subgrp["layer_ids"][()] if "layer_ids" in subgrp else None,
        )
