...
class MACECalculator(Calculator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.is_interlayer_calc:
            logging.info(
                "Initialized interlayer calculator; using atoms.arrays['layer_ids'] "
                "for cross-layer-only neighborhood construction"
            )
    ...
    def _atoms_to_batch(self, atoms):
        ...
        with torch_tools.default_dtype(self.default_dtype):
            config = mace_data.config_from_atoms(
                ...
                head_name=self.head,
                is_interlayer_atoms=self.is_interlayer_calc,
